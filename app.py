...
left_deck, right_display = st.columns()
...
with right_display:
    ...
    if run_verification and user_pasted_news.strip():
        ...
        # --- FIXED GAUGE CHART LOGIC MATRIX ---
        fig = go.Figure(go.Indicator(
            ...
            gauge={
                ...
                'steps': [
                    {'range': [0, 45], 'color': "rgba(219, 68, 85, 0.1)"},
                    {'range': [45, 75], 'color': "rgba(255, 193, 7, 0.1)"},
                    {'range': [75, 100], 'color': "rgba(29, 185, 84, 0.1)"}
                ],
            }
        ))
